...
def test_export_torchscript(end2end):
    ...
    file = DuoYOLO(MODEL).export(format="torchscript", optimize=False, imgsz=32, end2end=end2end)
...
def test_export_openvino(end2end):
    ...
    file = DuoYOLO(MODEL).export(format="openvino", imgsz=32, end2end=end2end)
    if WINDOWS:
...
def test_export_openvino_matrix(task, dynamic, int8, half, batch, nms, end2end):
    ...
    if WINDOWS:
        ...
        file = Path(file)
        file = file.rename(file.with_stem(f"{file.stem}-{uuid.uuid4()}"))
    DuoYOLO(file)([SOURCE] * batch, imgsz=64 if dynamic else 32, batch=batch)  # exported model inference
...
def test_export_onnx_matrix(task, dynamic, int8, half, batch, simplify, nms, end2end):
    ...
    DuoYOLO(file)([SOURCE] * batch, imgsz=64 if dynamic else 32)  # exported model inference
...
def test_export_coreml_matrix(task, dynamic, int8, half, nms, batch, end2end):
    ...
    DuoYOLO(file)([SOURCE] * batch, imgsz=32)  # exported model inference
...
def test_export_coreml():
    ...
    with redirect_stdout(stdout), redirect_stderr(stderr):
        DuoYOLO(MODEL).export(format="coreml", nms=True, imgsz=32)
        if MACOS:
            file = DuoYOLO(MODEL).export(format="coreml", imgsz=32)
            DuoYOLO(file)(SOURCE, imgsz=32)  # model prediction only supported on macOS for nms=False models
...
def test_export_paddle():
    ...
    DuoYOLO(MODEL).export(format="paddle", imgsz=32)
...
def test_export_mnn():
    ...
    file = DuoYOLO(MODEL).export(format="mnn", imgsz=32)
    DuoYOLO(file)(SOURCE, imgsz=32)  # exported model inference
...
def test_export_mnn_matrix(task, int8, half, batch, end2end):
    ...
    file = DuoYOLO(TASK2MODEL[task]).export(format="mnn", imgsz=32, int8=int8, half=half, batch=batch, end2end=end2end)
    DuoYOLO(file)([SOURCE] * batch, imgsz=32)  # exported model inference
    Path(file).unlink()  # cleanup
...
def test_export_ncnn():
    ...
    file = DuoYOLO(MODEL).export(format="ncnn", imgsz=32)
    DuoYOLO(file)(SOURCE, imgsz=32)  # exported model inference
...
def test_export_ncnn_matrix(task, half, batch):
    ...
    file = DuoYOLO(TASK2MODEL[task]).export(format="ncnn", imgsz=32, half=half, batch=batch)
    DuoYOLO(file)([SOURCE] * batch, imgsz=32)  # exported model inference
    shutil.rmtree(file, ignore_errors=True)  # retry in case of potential lingering multi-threaded file usage errors
