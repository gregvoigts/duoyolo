...
def test_tensorboard():
    ...
    DuoYOLO("yolo26n-cls.yaml").train(data="imagenet10", imgsz=32, epochs=3, plots=False, device="cpu")
...
def test_mlflow_keep_run_active():
    ...
    DuoYOLO("yolo26n-cls.yaml").train(data="imagenet10", imgsz=32, epochs=1, plots=False, device="cpu")
...
def test_triton(tmp_path):
    ...
    f = DuoYOLO(MODEL).export(format="onnx", dynamic=True)
    ...
    DuoYOLO(f"http://localhost:8000/{model_name}", "detect")(SOURCE)  # exported model inference
...
def test_faster_coco_eval():
    ...
    from duoYolo.models.yolo import DetectionValidator
